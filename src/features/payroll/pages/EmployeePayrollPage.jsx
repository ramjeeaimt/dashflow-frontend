 ...
 sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-60'
        ...
        <main className="flex-1 mt-16 overflow-y-auto p-4 md:p-8 space-y-6 pb-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-foreground tracking-tight">Payroll History</h1>
              <p className="text-sm text-muted-foreground">Welcome, {user?.name || "Employee"}. View your earnings summary.</p>
            </div>
            <button className="flex items-center gap-2 px-4 py-2 bg-card border border-border text-xs font-semibold text-foreground hover:shadow-md transition-all rounded-xl">
                ...
                <div className="flex bg-muted p-1 ">
                  {["All", "Paid", "Pending"].map((tab) => (
                    <button
                      ...
                      className={`px-6 py-1.5 text-[11px] font-semibold uppercase transition-all ${
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      className="appearance-none pl-3 pr-8 py-2 bg-muted/60 border border-border text-xs font-bold text-foreground focus:ring-2 focus:ring-ring outline-none cursor-pointer"
                    >
                    ...
                    <ChevronRight size={14} className="absolute right-2 top-1/2 -translate-y-1/2 rotate-90 text-muted-foreground/70 pointer-events-none" />
                  </div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-24 pl-3 pr-2 py-2 bg-muted/60 border border-border rounded-xl text-xs font-bold text-foreground focus:ring-2 focus:ring-ring outline-none"
                      value={selectedYear}
                  ...
                  <div className="relative w-full md:w-64">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2 bg-muted/60 border border-border rounded-xl text-xs font-bold focus:ring-2 focus:ring-ring outline-none"
                      value={searchTerm}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-muted/50 border-b border-border">
                  <tr className="text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-wide">
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-muted flex items-center justify-center text-muted-foreground group-hover:bg-primary/90 group-hover:text-white transition-all">
                              <span className="text-[10px] font-semibold">{monthNames[p.month - 1]}</span>
                            </div>
                            <div>
                              <p className="text-sm font-bold text-foreground">{monthNames[p.month - 1]} {p.year}</p>
                              <p className="text-[9px] text-muted-foreground/70 font-bold uppercase tracking-tighter">REF: {p.id.slice(-8)}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="text-sm font-semibold text-foreground tracking-tight">₹{Number(p.netSalary).toLocaleString('en-IN')}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-tight ${
 p.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                    ...
                    <tr>
                      <td colSpan="4" className="p-20 text-center">
                        ...
                        <p className="text-muted-foreground/70 font-bold italic text-sm">No payroll records match these filters.</p>
                      </td>
                    </tr>
                  )}
                </tbody>
...
const StatCard = ({ title, value, icon, color, loading }) => (
  <div className={`bg-card p-5 border border-border flex items-start justify-between`}>
    <div>
      <p className="text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-wide">{title}</p>
      {loading ? <div className="h-7 w-24 bg-muted/60 animate-pulse rounded mt-2" /> : <h3 className="text-xl font-semibold text-foreground mt-1">{value}</h3>}
    </div>
...
const DetailRow = ({ label, value, isRed }) => (
  <div className="flex justify-between border-b border-slate-50 pb-2">
    <span className="text-[10px] text-muted-foreground/70 font-semibold uppercase tracking-wide">{label}</span>
    <span className={`text-sm font-bold ${isRed ? 'text-red-500' : 'text-foreground'}`}>{value}</span>
  </div>
);
  ...
  <>
    {[1, 2, 3].map((i) => (
      <tr key={i} className="animate-pulse">
        <td className="px-6 py-4"><div className="h-12 w-full bg-muted/60 rounded-xl" /></td>
        <td className="px-6 py-4"><div className="h-6 w-20 bg-muted/60 rounded" /></td>
        <td className="px-6 py-4"><div className="h-6 w-24 bg-muted/60 rounded-full" /></td>
        <td className="px-6 py-4"><div className="h-8 w-8 bg-muted/60 rounded ml-auto" /></td>
      </tr>
    ))}
  </>
);
