...
const RewardSystemSettings = () => {
  ...
  if (loading) return <div className="p-8 text-sm text-muted-foreground/70">Loading reward settings…</div>;

  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-foreground">Employee Reward System</h3>
        <p className="text-sm text-muted-foreground mt-1">Motivate employees with a points-based reward system. Employees earn points for on-time attendance and lose points for late arrivals.</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden mb-6">
        <div className="px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">Enable Reward System</p>
              <p className="text-xs text-muted-foreground/70 mt-0.5">Turn the employee reward system on or off for this company</p>
            </div>
          </div>
          <Toggle
      ...
      <div className={`bg-card border border-border rounded-xl shadow-sm overflow-hidden mb-6 transition-opacity ${!form.enableRewardSystem ? 'opacity-40 pointer-events-none' : ''}`}>
        <div className="px-6 py-4 border-b border-border flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center">
            <Icon name="TrendingUp" size={16} className="text-emerald-600" />
          </div>
          <div>
            <p className="text-sm font-semibold text-foreground">Points Configuration</p>
            <p className="text-xs text-muted-foreground/70">Set how points are earned and deducted</p>
          </div>
        </div>
        <div className="px-6">
          <FieldGroup
      ...
      <div className={`bg-card border border-border rounded-xl shadow-sm overflow-hidden mb-6 transition-opacity ${!form.enableRewardSystem ? 'opacity-40 pointer-events-none' : ''}`}>
        <div className="px-6 py-4 border-b border-border flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Reward Description</p>
            <p className="text-xs text-muted-foreground/70">Tell employees what they can earn</p>
          </div>
        </div>
        <div className="px-6 py-5">
          <textarea
            ...
            className="w-full px-4 py-3 text-sm border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-ring disabled:bg-muted/60 disabled:text-muted-foreground/70 resize-none"
          />
          <p className="text-xs text-muted-foreground/70 mt-2">This message is shown to employees when they view their reward points balance.</p>
        </div>
      </div>
